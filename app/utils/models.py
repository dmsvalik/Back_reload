...
class GallerySlider(models.Model):
    ...
    name = models.CharField("название слайдера по позиции в макете", max_length=11, choices=SLIDER_NAME, unique=True)
...
class GalleryImages(models.Model):
    ...
    id = models.AutoField(primary_key=True, unique=True)
    slider = models.ForeignKey(GallerySlider, on_delete=models.CASCADE, null=True)
    name = models.CharField("название для картинки", max_length=120, null=True, blank=True)
    price = models.CharField("цена за которую был выполнен заказ", max_length=10, null=True, blank=True)
    position = models.CharField("место на макете - слева направо", max_length=11, choices=IMAGE_PLACE, default="1")
