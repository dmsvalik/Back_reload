...
class UserQuotaManager:
    ...
    def __init__(self, user: UserAccount):
        ...
        self.user_quota: UserQuota = UserQuota.objects.filter(user=self.user).first()
    ...
    def add_many(self, files: QuerySet[OrderFileData | FileData]) -> UserQuota:
        ...
        """
        sizes: dict = files.aggregate(cloud_size=Sum("yandex_size"), server_size=Sum("server_size"))
        (
            UserQuota.objects
            .filter(pk=self.user_quota.pk)
            .update(
                total_cloud_size=F("total_cloud_size")+sizes.get("cloud_size"),
                total_server_size=F("total_server_size")+sizes.get("server_size")
                )
            )
